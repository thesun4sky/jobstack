...
| **구독·결제** | **Stripe 또는 PortOne** — 클라이언트에서 결제 UI, **웹훅으로만** 구독 상태 확정. 저장소에는 PortOne 웹훅 예시 경로(`app/api/webhooks/portone/`)가 있다. Phase 3에서 `subscriptions`·`usage_events`로 API·스킬 실행 게이팅 |
...
| Phase | 초점 | 기술적으로의 의미 |
|-------|------|-------------------|
| **1** | 웹 뼈대, 인증, 스킬 1~2개 **수직 슬라이스** | `apps/web`(Next.js), BFF Route Handlers, 최소 Job/세션 API |
| **2** | 나머지 스킬 포팅, 상태 DB화, `jobstack-view` 웹 대체 | 13 스킬 레지스트리·워커, 문서/트래커 UI, ERD·REST 정식화 |
| **3** | 결제·플랜·게이팅 | Stripe/PortOne, 웹훅, 쿼터·사용량 |

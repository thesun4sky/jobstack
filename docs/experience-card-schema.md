...
`basis`·`source`가 비면 `apply`가 저장을 거부한다 — 근거 없는 R 은 §1 의 사실 날조와 같은 취급이다. 회사당 1건이므로 같은 회사(정규화 기준)로 다시 `apply`하면 항목을 교체하고 `created_at`도 새 값이 된다. 항목 삭제 명령은 두지 않는다(후속 후보).

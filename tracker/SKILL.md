...
해당 회사의 기업분석 캐시(`$_JS_STATE/company-cache/`)가 있으면 company-research 캐시 요약의 `적합도 종합:` 정수(세 점수 평균 반올림 — `references/…` 가 아니라 캐시 파일 값, 미산출이면 생략)를 `--fit-score`·`--research-ref`로 함께 넘깁니다. 캐시가 없거나 공고 본문 미확보로 점수가 없으면 두 옵션을 생략합니다(추정치 금지).
...
출력을 **코드 블록으로 그대로 인용**합니다(요약·의역 금지). 진행중·대기중·종료 세 그룹으로 묶고 그룹 안에서는 마감일/업데이트 순으로 정렬하며, 총 5건 이하면 `▸` 축약 표기로 자동 전환됩니다(모바일 가독성). 하단 `통과율:` 줄은 `${CLAUDE_SKILL_DIR}/references/tracker-states.md`의 퍼널 규칙(`max_stage` 기준, withdrawn 분모 제외)을 반영하며, 구버전·폴백 건이 섞여 있으면 정확도 제한 안내가 함께 붙습니다.
...
1. **채용서류 반환 청구권** — 반환 의무 기한·청구 가능 기간은 실행 시 WebSearch로 채용절차법 현행 조문을 확인해 출처·기준일과 함께 안내합니다(수치를 외워 적지 않습니다).
...
단서:
- 채용절차법은 사업장 규모에 따라 적용 범위가 다르므로 적용 기준(상시 근로자 수)도 실행 시 함께 확인합니다.
- 관련 법 개정 여부(공정채용법 추진 등)는 단정하지 말고, 실행 시 WebSearch로 현행 조문·시행 여부를 출처·기준일과 함께 확인합니다.
- 항목별 확인 요령(개인정보 파기 요청·면접비 등)과 지원 시 정책 인센티브 목록은 `${CLAUDE_SKILL_DIR}/references/policy-checklist.md`를 Read 해 같은 규칙(출처·기준일 병기, 수치 단정 금지)으로 안내합니다.
...
출력을 **코드 블록으로 그대로 인용**합니다(요약·의역 금지 — `퍼널 전환율`·분포 수치 그 자체가 근거입니다). 그 아래에 한두 줄 코칭만 덧붙입니다. 상태별 분포, 퍼널 전환율(`${CLAUDE_SKILL_DIR}/references/tracker-states.md`의 `max_stage` 기준·withdrawn 분모 제외 규칙), 평균 진행 기간을 스크립트가 계산합니다. `fit_score` 상·하위 서류합격률 비교는 상·하위 각 그룹에 결과가 확정된 건이 3건 이상일 때만 스크립트가 마지막 줄에 자동으로 덧붙입니다(미만이면 생략 — 소표본 노이즈 방지).

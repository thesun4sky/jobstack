...
- **입력 방식**: 파일 경로를 주면 Read로 읽고, 텍스트를 붙여넣으면 그대로 정식 입력으로 인정합니다. URL만 주는 경우 로그인 담벼락으로 본문 확보가 어려우므로 `${CLAUDE_SKILL_DIR}/references/guardrails.md` §2에 따라 "프로필 텍스트를 붙여넣어 주시면 바로 진행합니다"로 자료 요청으로 전환합니다(도구 한계를 그대로 노출하지 않습니다).

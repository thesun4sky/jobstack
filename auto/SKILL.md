...
> **파일 출력 선택지 (모든 Case 공통)**: 각 Case의 선택지에 "최종 파일 출력(.docx)"을 포함합니다. `${CLAUDE_SKILL_DIR}/../bin/jobstack-export`가 있으면 pandoc으로 md→docx 변환하고, 없거나 변환 실패(exit 2) 시 markdown 본문을 복붙용으로 제공하는 폴백으로 진행합니다. (봇 환경에서는 기존 File output protocol을 따릅니다.)
...
2. **이력서·경력기술서형** — 이력서/경력기술서만 요구 → 이력서·포트폴리오 방향으로 추천 전환, 자소서 선택지 비노출.

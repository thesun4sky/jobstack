...
allowed-tools:
  ...
  - AskUserQuestion
  - WebSearch
argument-hint: "[모드: 인성|PT|토론|AI|기술|임원|컬처핏] [회사명] [강도]"
when_to_use: |
...
5. **경험 카드** (있으면): `"$_JS_BIN/jobstack-exp.mjs" list --company <회사명>`·`show <id>` (env.sh 소싱 후)로 카드의 `change`·`numbers`를 답변 골자·수치 검증 질문의 근거로, 지원 기업과 일치하는 `apply_plans`(입사 후 적용 문장)를 "입사하면 이 경험을 어떻게 쓰겠나" 류 질문·답변 평가의 근거로 씁니다(`references/experience-card-schema.md` 계약).
...
| 모드 | 파일 |
|------|------|
| A) 인성면접 | `references/modes/personality.md` |
| B) PT면접 | `references/modes/pt.md` |
| C) 토론면접 | `references/modes/debate.md` |
| D) AI면접·AI역량검사 대비 | `references/modes/ai.md` |
| E) 기술면접 | `references/modes/tech.md` |
| F) 임원면접 | `references/modes/exec.md` |
...
**defense-map 갱신**: 면접에 defense-map을 사용했다면, 종료 후 각 entry의 `defense_status`를 실전 답변 품질로 갱신합니다 (`ready`/`weak`/`unprepared`). 갱신은 `"$_JS_BIN/jobstack-defense-map.mjs" set-status --company <회사명> <entry-id> <ready|weak|unprepared>` (env.sh 소싱 후)로 하며 파일을 직접 편집하지 않습니다.

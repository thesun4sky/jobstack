...
allowed-tools:
  ...
  - WebSearch
  - WebFetch
argument-hint: "[직무 키워드 | 공고 URL] [--platform wanted|saramin|jobkorea|jumpit]"
when_to_use: |
  희망 직무·조건에 맞는 채용공고를 사람인·잡코리아·원티드·점핏에서 찾거나, 공고 URL·원문을 매칭도 분석과 수시·공채 캘린더로 정리할 때 쓴다.
  특정 기업 자체를 조사할 때는 `/company_research`, 채용 전략을 세울 때는 `strategy` 스킬, 지원 이후 진행 상태 관리는 `tracker` 스킬로 넘긴다.
  이미 확보한 공고를 바탕으로 이력서·자소서를 쓸 때는 `/resume`·`/cover_letter` 를 쓴다.
metadata:
  preamble-tier: 2
  version: 0.5.0
...
!`bash "${CLAUDE_SKILL_DIR}/scripts/preamble.sh" job-search "${CLAUDE_SESSION_ID}" "${CLAUDE_PLUGIN_DATA:-}"`
...
> 추출 체크리스트: `${CLAUDE_SKILL_DIR}/references/analysis.md` — 공고 본문을 확보한 직후, 정보를 표로 정리하기 전에 Read 한다.
...
> 가중치·등급 세부: `${CLAUDE_SKILL_DIR}/references/matching.md` — 스코어를 실제로 계산하기 직전에 Read 한다.
...
> 출력 형식 예시: `${CLAUDE_SKILL_DIR}/references/calendar-template.md` — 캘린더를 실제로 출력하기 직전에 Read 한다.

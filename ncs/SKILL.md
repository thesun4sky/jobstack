...
- STAR-R 기법(`${CLAUDE_SKILL_DIR}/references/experience-methods.md` §7)과 NCS 역량 단위를 연결 — 두 번째 R(입사 후 적용)은 기관 직무기술서·인재상 등 **기관 분석 근거**가 있을 때만 씁니다

**STAR의 한계:** STAR는 구조일 뿐입니다. 문제 정의·선택 이유·역할·전후 차이가 없으면 약합니다. '저희는/우리 팀은'으로 시작하는 문장은 본인 역할이 묻히므로, '저는 그중 ~을 맡아'로 역할을 분리했는지 점검합니다. R(입사 후 적용)도 같은 기준입니다 — 기관명을 다른 기관으로 바꿔도 성립하면 다시 씁니다(공공기관은 자소서 재활용이 감점 요인).

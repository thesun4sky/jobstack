...
- YAML 프론트매터: `name`, `description`, `allowed-tools`, `argument-hint`, `when_to_use` (Claude Code 표준) + `metadata:` 아래 `preamble-tier`, `version`, `benefits-from` (jobstack 메타). SKILL.md 는 300줄 이하(`test/test-skill-size.sh`) — 흐름·게이트·판단 규칙만 두고 정적 자료는 `references/` 로 옮겨 필요한 시점에 Read 한다(진행적 공개)

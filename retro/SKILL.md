...
**회고 완료 이벤트 기록:** 회고를 마치면 아래 후속 이벤트 1줄을 `skill-usage.jsonl`에 append합니다(고정 태그·상태값만 기록, 자유 텍스트·제3자 PII 미포함 — 서비스 사용자 데이터는 익명·집계만).

```bash
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","pid":'$$',"event":"done","mode":"B","stage":"1차면접","tags":["꼬리질문대응","수치화부족"]}' \
  >> "$_JS_STATE/analytics/skill-usage.jsonl" 2>/dev/null || true
```
- `mode`는 Phase 1에서 고른 회고 모드(A/B/C), `stage`는 B 모드일 때 탈락 단계(그 외 생략 가능), `tags`는 이번 회고의 weakness_tags입니다. 규격은 `docs/telemetry-events.md`를 따릅니다.
